...
function StartingGame()
{
  ...
  textSize(30);
  fill("RED");
  text("C-VIRUS",displayWidth/2,displayHeight/9);
  text("CLICK ON THE EQUIPMENT TO ATTACK THE VIRUS",displayWidth/2-350,displayHeight/6+50);
  if (abc === 0)
  {
  ...
  if(Virus1.collide(Mask))
  {
    Virus1.x = displayWidth/2+350;
    Virus1.y = displayHeight/2-50;
    Mask.velocityX = 0;
    Mask.velocityY = 0;
    Virus1.velocityX = 0;
    virus1health = virus1health+10;
...
function Winning()
{
  if(virus1health <= 0)
  {
    ...
    fill("RED");
    text("C-VIRUS",displayWidth/2,displayHeight/9);
    text(" YOU WINNN !!!",displayWidth/2-100,displayHeight/2);
    Mask.destroy();
    Sanitizer.destroy();
    Immubar.destroy();
    Virus1.destroy();
